...
execute store success score $Clicked bubblewaves run clear @p[scores={bubblewaves=1}] structure_void{BUWS:{id:"gui:retinal_scanner"}} 0
execute if score $Clicked bubblewaves matches 1 run function bubblewaves:blocks/bubble_bench/gui/security/retinal_scanner